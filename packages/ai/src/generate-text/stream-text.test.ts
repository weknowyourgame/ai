  ...
  describe('programmatic tool calling', () => {
    ...
    describe('deferred tool calls with tool-error', () => {
      ...
      it('should resolve deferred tool call when tool-error arrives in a later step', async () => {
        // Realistic programmatic tool calling: the deferred provider tool calls
        // a client tool, whose output is sent back and drives the continuation.
        // The deferred provider tool-error then arrives in the next step.
        let responseCount = 0;
        const result = streamText({
          model: new MockLanguageModelV4({
            doStream: async () => {
              switch (responseCount++) {
                case 0:
                  // Step 1: deferred provider tool call + client tool call (no
                  // provider result yet - it is deferred)
                  return {
                    stream: convertArrayToReadableStream([
                      {
                        type: 'response-metadata',
      ...
      it('should not run an extra step when a deferred provider tool is pending but there is no client output to send back', async () => {
        // Regression: a deferred provider tool call with no client tool output
        // to send back must not trigger another model call. Continuing would
        // build a next request ending with an assistant message (no following
        // tool message), which providers like Anthropic reject with
        // "assistant message prefill not supported".
        let responseCount = 0;
        const result = streamText({
          model: new MockLanguageModelV4({
            doStream: async () => {
